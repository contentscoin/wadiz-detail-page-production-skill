...
- 상품: 스윙핏 리필 골프볼 구독 서비스
- 목적: 스마트스토어용 모바일 상세페이지
- 권장 컷 수: 12컷
...
|---:|---|---|---|---|
| 1 | 첫 후킹 | 라운드 전 골프공 준비 스트레스 감소 | 골프백, 티박스, 새 골프공 패키지의 프리미엄 hero scene | 보장/1위 표현 금지 |
| 2 | 문제 공감 | "라운드 잡힐 때마다 공부터 챙기고 있나요?" | 비어 있는 골프공 파우치와 준비 체크리스트 | 공감형 질문, 과장 금지 |
| 3 | 해결 제안 | 매월 필요한 만큼 도착하는 골프공 구독 | 택배 박스 개봉 컷, 제품 패키지 정돈 | 배송 주기 fact 확인 |
| 4 | 사용 방식 | 옵션 선택 -> 정기 배송 -> 라운드 준비 | 3단계 인포그래픽 | 텍스트 밀도 낮게 |
| 5 | 제품 디테일 | 딤플, 마킹, 패키지 디테일 강조 | 골프공 macro render, 로고/마킹 영역 | 실제 마킹 가능 범위 확인 |
| 6 | 옵션 비교 | 라운드 빈도에 맞춘 구성 | 옵션 카드 3개 | 가격/구성 미확인 시 placeholder |
| 7 | 선물/브랜드 활용 | 동호회, 회사 행사, 선물용 활용 | 패키지와 메시지 카드 scene | B2B 가능 여부 확인 |
| 8 | 불안 해소 | "정기구독이 부담스럽지 않도록" | 해지/변경/배송 안내 UI mock | 실제 정책 없으면 확정 표현 금지 |
| 9 | 후기/사용 상황 | 라운드 준비 루틴에 자연스럽게 들어오는 서비스 | 골프장 락커룸, 자동차 트렁크 준비 scene | 실제 후기 없으면 후기처럼 쓰지 않기 |
| 10 | FAQ | 배송, 옵션 변경, 마킹, 교환 문의 | FAQ accordion 스타일 이미지 | 정책 fact와 일치 |
| 11 | 구매 CTA | 첫 배송 구성과 선택 옵션 안내 | 패키지 full set + CTA zone | 가격 anchor 확인 |
| 12 | 마지막 리마인드 | "다음 라운드 준비를 더 가볍게" | 해질녘 그린 위 제품 hero scene | 감성 카피, 과장 claim 없음 |
...
- "실력 향상", "스코어 보장", "손실 보상"처럼 결과를 보장하는 표현은 사용하지 않는다.
- "국내 최초", "업계 1위", "최저가"는 공식 증빙 없으면 사용하지 않는다.
- 정기구독 해지/변경/환불은 실제 정책 문구가 확인되기 전까지 확정형으로 쓰지 않는다.
- 후기나 만족도는 실제 리뷰 데이터 없으면 만들지 않는다.
